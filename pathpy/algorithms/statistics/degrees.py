...
# Time-stamp: <Wed 2020-03-31 13:23 scholtes>
#
...
def assortativity(network, weight: bool = False) -> float:
    ...
    for i in network.nodes:
        for j in network.nodes:
            cov += (A[idx[i], idx[j]] - (network.nodes.degrees(weight)[i]*network.nodes.degrees(weight)[j])/(2*m)) * network.nodes.degrees()[i] * network.nodes.degrees()[j]
            if i!=j:
                var -= (network.nodes.degrees(weight)[i]*network.nodes.degrees(weight)[j])/(2*m) * network.nodes.degrees(weight)[i] * network.nodes.degrees(weight)[j]
            else:
                var += (network.nodes.degrees(weight)[i] - (network.nodes.degrees()[i]*network.nodes.degrees()[j])/(2*m)) * network.nodes.degrees()[i] * network.nodes.degrees()[j]
    return cov/var