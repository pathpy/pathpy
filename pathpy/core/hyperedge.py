...
class EdgeCollection(BaseCollection):
    ...
    def _add_edge(self, *edge: Union[str, tuple, list, Node, Edge],
        ...
        elif all(isinstance(arg, set) for arg in edge) and nodes:
            _sets = [cast(Set[Any], node) for node in edge]
            self._add_edge_from_set(*_sets, uid=uid, **kwargs)
    ...
    def _add_edge_from_set(self, *sets: set, uid: Optional[str] = None,
                           **kwargs: Any) -> None:
        """Helper function to add an hyperedge from a set"""
        _nodes: list = []
        ...
        if (_nodes[0], _nodes[1]) not in self or self.multiedges:
            self._add_edge(Edge(_nodes[0], _nodes[1], uid=uid,
                                hyperedge=self._hyperedges, **kwargs))
